    ...
    */
    ...
    if(req.query.instrumentTypes){
        const condition = {
            ...
            //through: { attributes: [] }  //Doesn't work. Deprecated?
            attributes:[] //works
    ...
    if(req.query.musicianFields){
        if(req.query.musicianFields === "all"){
            ...
            //query.attributes = [] //But SQL request doesn't work in such case so i guess i do not understand the problem
            query.attributes = ['id'] //works but give the attribute id so not "Do not include ANY musician attributes"
        }else {
            query.attributes = [...req.query.musicianFields] // Works
        }

        //No need to do anything for "all" because it will select everything by default
    }
